...
import { createRating, getMyRatings } from "../services/ratingService";
    ...
    try {
      ...
      const ratedBookingIds = new Set(
        (ratingsData?.ratings || []).map((rating) =>
          rating.booking?._id ? rating.booking._id : rating.booking
        )
      );

      const completedBookings = (bookingsData?.bookings || [])
        .filter((booking) => booking.status === "completed")
        .filter((booking) => !ratedBookingIds.has(booking._id))
        .sort(
          (a, b) =>
            new Date(b.preferredDate || b.createdAt) -
            new Date(a.preferredDate || a.createdAt)
        );
        ...
        >
          <div className="text-center mb-12">
            ...
            <p className="mt-3 text-slate-500 font-medium max-w-lg mx-auto">
              Your feedback helps us maintain the highest quality of service in
              our community.
            </p>
          </div>
          ...
          {loading ? (
            ...
            >
              ...
              <h2 className="text-2xl font-bold text-slate-900 mb-2">
                No Service to Rate Yet
              </h2>
              <p className="text-slate-500 mb-8 max-w-md mx-auto">
                Rating is only available for services marked as completed and
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              <div className="relative group">
                <label className="text-sm font-bold text-slate-700 ml-1 mb-3 block tracking-wide uppercase">
                  ...
                  >
                    {bookings.map((booking) => (
                      <option key={booking._id} value={booking._id}>
                        ...
                        {booking.issue?.category?.toUpperCase() || "SERVICE"} •{" "}
                        {booking.preferredDate || "No date"}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="flex flex-col items-center">
                <label className="text-sm font-bold text-slate-700 mb-6 tracking-wide uppercase">
